...
for(i in 1:length(temp1$gene)){
  ...
  text(-17, 3.5, "bLR", cex=1.8)
  mtext(paste(temp1$gene[i]), line=-1.5, cex=2.5)
  text(7, 2.7, "Cohen's D [95% CI]", cex=1.6)
  text(10, 3.5, "bHR", cex=1.8)
  dev.off()
}


#Megan's Code to try to make the Ncan P7 Forest Plots match the P14:

library(metafor)
temp1<-subset(P7Meta, P7Meta$gene=="Ncan")

  tiff("Ncan P7 Forest Plot.tiff", res=300, compression="lzw",
  ...
  forest.rma(rma.mv(effect, var), slab=c("MBNI_AffymetrixRgU34A_F6", "Alabama_NimbleGen_F34"), 
             xlim=c(-21, 14), cex=1.6)
  text(-16, 3.5, "Investigator & Study", cex=1.6)
  text(-19, 4.5, "bLR", cex=1.8)
  mtext(paste("P7\n", temp1$gene[i]), line=-1.5, cex=2.5)
  text(9, 3.5, "Cohen's D [95% CI]", cex=1.6)
  text(11, 4.5, "bHR", cex=1.8)
  dev.off()
...
#Megan's Code to try to make P14 forest plots with different formatting:

library(metafor)
temp1<-subset(P14Meta, P14Meta$gene=="Ncan"| P14Meta$gene=="Bmp4"| P14Meta$gene=="C1qa" | P14Meta$gene=="C1qb" | P14Meta$gene=="C1qc"| P14Meta$gene=="Sox9")

outGOIPlots<-"/Users/mhh/Documents/Microarray Gen/HRLR/ThesisMetaAnalysisOutput/ForestPlots/Prettier/"

for(i in 1:length(temp1$gene)){
  
  tiff(paste0(outGOIPlots, "P14 Forest Plot ", paste((temp1$gene)[i]), ".tiff"), res=300, compression="lzw", width = 8.5, height = 8, units = 'in')
  
  effect<-c(temp1$`d.P14`[i], temp1$`d.affy`[i], temp1$`d.illumina`[i], temp1$`d.RNAseq`[i], temp1$`d.new.colony.P14`[i])
  var<-c(temp1$`var.d.P14`[i], temp1$`var.d.affy`[i], temp1$`var.d.illumina`[i], temp1$`var.d.RNAseq`[i], temp1$`var.d.new.colony.P14`[i])
  
  forest.rma(rma.mv(effect, var), slab=c("MBNI_AffymetrixRgU34A_F6", "MBNI_AffymetrixRae230_F15", "MBNI_IlluminaRatRef12v1_F15","MBNI_RNASeq_F29", "Alabama_NimbleGen_F34"), xlim=c(-20, 13), cex=1.6)
  text(-13, 5.6, "Investigator & Study", cex=1.6)
  text(-16, 6.6, "bLR", cex=1.8)
  mtext(paste("P14\n", temp1$gene[i]), line=-1.5, cex=2.5)
  text(6, 5.6, "Cohen's D [95% CI]", cex=1.6)
  text(9, 6.6, "bHR", cex=1.8)
  dev.off()
}
...
#Megan's adaptation of the cell type heatmap code to make it match the color scheme in the other figures:

#Need dataframe of all betas for cell types - rows cell types, columns are age groups

setwd("~/Documents/Microarray Gen/HRLR/Code_Isabelle/SecondaryInput")

#Reading in cell type meta output from adult analysis
cellTypeAdult <- read.csv("Adult_CellTypeMeta_CohDandPval.csv", header=T, stringsAsFactors = F)
colnames(cellTypeAdult)


CellType_MetaAnalysisOutputP7<-read.csv("CellType_MetaAnalysisOutputP7.csv", header=T, stringsAsFactors = F)
str(CellType_MetaAnalysisOutputP7)

CellType_MetaAnalysisOutputP14<-read.csv("CellType_P14Output_Pval_CohD.csv", header=T, stringsAsFactors = F)
str(CellType_MetaAnalysisOutputP14)

CellType_MetaAnalysisOutputP21<-read.csv("CellType_MetaAnalysisOutputP21.csv", header=T, stringsAsFactors = F)
str(CellType_MetaAnalysisOutputP21)

#Creating data frame of cell type estimated effects (Betas)

cellTypeBetas <- cbind.data.frame(CellType_MetaAnalysisOutputP7[,2], CellType_MetaAnalysisOutputP14[,2],
                                  CellType_MetaAnalysisOutputP21[,2], cellTypeAdult$estimate)

str(cellTypeBetas)
row.names(cellTypeBetas)<-cellTypeAdult$X

colnames(cellTypeBetas) <- c("P7", "P14", "P21", "Adult")

#do not want neuron cell types
...
#All Ages:
tiff("CellTypeHeatmap2.tiff", 
     width = 16, height = 6, units = 'in', res = 300, compression = "lzw")

par(mar=c(5,16,4,5))
image(t(as.matrix(cellTypeBetas)), axes=F, col=colorpanel(64, "red", "white", "green3"))
axis(2, at=c(0, .166, .33, .5, .67, .84, 1), labels=row.names(cellTypeBetas), las=1, cex.axis=1.4)
axis(1, at=c(0, .33, .67, 1), labels=colnames(cellTypeBetas), las=2, cex.axis=1.5)
...
#Adding color gradient legend
image.plot(t(as.matrix(cellTypeBetas)), axes=F, col=colorpanel(64, "red", "white", "green3"), 
           legend.only=T, axis.args = list(cex.axis = 1.5))
...
#Just P14 and Adult
tiff("CellTypeHeatmap3.tiff", 
     width = 6, height = 6, units = 'in', res = 300, compression = "lzw")

par(mar=c(5,16,4,5))
image(t(as.matrix(cellTypeBetas[,c(2,4)])), axes=F, col=colorpanel(64, "red", "white", "green3"))
axis(2, at=c(0, .166, .33, .5, .67, .84, 1), labels=row.names(cellTypeBetas), las=1, cex.axis=1.4)
axis(1, at=c(0, 1), labels=colnames(cellTypeBetas[,c(2,4)]), las=2, cex.axis=1.5)

# Add * symbol for significant cell typse
points(1, .33, pch=8, cex=2) #adult microglia
points(0, .33, pch=8, cex=2) #P14 microglia
points(0, .17, pch=8, cex=2) #P14 endothelial
#mtext("Cell Type Estimate by Age", cex=2.2, line=1)
